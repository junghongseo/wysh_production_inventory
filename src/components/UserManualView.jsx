...
const UserManualView = () => {
  ...
  const manualData = useMemo(() => [
    ...
    // -------------------------------------------------------------
    {
      ...
      content: [
        ...
        },
        {
          ...
          items: [
            { term: '목표 수율 (Target Yield %)', desc: '플레이버 제품 생산 시 연동된 베이스 요거트의 설정 수율(예: 31.5%)이 자동으로 연동되어 목표치 및 원재료 필요량으로 산출됩니다.' },
    ...
    // -------------------------------------------------------------
    {
      ...
      categoryId: 'order',
      mode: 'all',
      title: '📁 1. 주문서 엑셀 정리 및 공식 출고표 반영',
      summary: '주문 엑셀 파일(.xlsx, .xls)을 업로드하여 포장 유형별 수량 집계 및 공식 출고표를 클라우드에 반영합니다.',
      tags: ['주문서 엑셀 정리', '포장 유형 집계', '공식 출고표 반영', '수량별 구성비', '클라우드 저장'],
      content: [
        {
          heading: '📁 엑셀 파일 업로드 및 자동 분석',
          items: [
            { term: '📂 엑셀 주문서 업로드', desc: '스마트스토어, 자사몰 등의 엑셀 배송/주문 파일(.xlsx, .xls)을 드래그 앤 드롭하거나 클릭하여 선택합니다.' },
            { term: '⚡ 자동 정제 & 포장 유형 분류', desc: '상품명에서 중량/용량 표기를 자동 정제하고, 동일 주문번호(송장번호)의 품목들을 포장 조합별로 묶어 건수 및 비율을 집계합니다.' },
            { term: '📊 수량별 주문 구성 요약', desc: '주문건당 구매 수량 기준 1개 주문, 2개 주문, 3개 이상 주문건의 건수와 백분율(%)을 상단 뱃지로 한눈에 보여줍니다.' }
          ]
        },
        {
          heading: '📌 공식 출고표 반영 (관리자 승인 저장)',
          items: [
            { term: '📅 반영 날짜 지정', desc: '기본값은 오늘 날짜이며, 원하는 날짜를 선택하면 제목(예: 26-07-27 출고표)이 자동 구성됩니다.' },
            { term: '📌 [공식 출고표로 반영하기] 버튼', desc: '중간 확인용 엑셀 업로드와 구분하여, 작업자들이 볼 수 있는 그날의 최종 출고표로 Supabase 클라우드 DB에 영구 반영합니다.' }
          ]
        }
      ]
    },
    {
      ...
      categoryId: 'order',
      mode: 'all',
      title: '📋 2. 일별 공식 출고표 조회, 인쇄 및 이미지 다운로드',
      summary: '날짜별로 저장된 공식 출고표를 조회하고 출고 현장 전달용 인쇄 및 PNG 이미지 다운로드를 수행합니다.',
      tags: ['공식 출고표 조회', '일별 출고표', '인쇄', 'PNG 다운로드', '특이사항 수정'],
      content: [
        {
          heading: '🔍 날짜별 공식 출고표 조회',
          items: [
            { term: '📅 조회 일자 선택 & 퀵 칩', desc: '달력 컨트롤 또는 상단 등록목록 칩을 클릭하여 원하는 날짜의 공식 출고표를 즉시 불러옵니다.' },
            { term: '📦 제품별 총 발송 수량', desc: '해당 출고표에서 발송해야 하는 각 제품별 총 수량(개수)을 일목요연하게 표시합니다.' },
            { term: '📝 특이사항 입력 & 클라우드 동기화', desc: '출고표 하단 특이사항을 수정한 후 [💾 저장] 버튼을 누르면 클라우드 DB에 실시간 반영됩니다.' }
          ]
        },
        {
          heading: '🖨️ 인쇄 및 이미지 파일 저장',
          items: [
            { term: '🖨️ [인쇄] 버튼', desc: '출고표 포맷 전용 인쇄 창을 열어 작업장 프린터로 즉시 출력합니다.' },
            { term: '🖼️ [이미지 저장] 버튼', desc: '출고표 화면을 고화질 PNG 이미지 파일(`출고표_YYYYMMDD.png`)로 자동 생성하여 다운로드합니다.' }
          ]
        }
      ]
    },
    {
      ...
      categoryId: 'order',
      mode: 'all',
      title: '📊 3. 출고 동향 & 수량별 비중 분석 (차트 & 세부 데이터)',
      summary: '공식 출고표 데이터를 기반으로 일별 총 출고량 추이, 수량별 비중 원형(도넛) 차트, 세부 통계 표를 조회합니다.',
      tags: ['출고 동향', '선 그래프', '도넛 차트', '조회 기간 필터', '페이지네이션', '일평균'],
      content: [
        {
          heading: '📈 출고 동향 및 구성 비중 차트',
          items: [
            { term: '🗓️ 조회 기간 선택', desc: '[최근 7일], [최근 14일], [최근 30일], [전체] 버튼을 통해 통합 분석 기간을 지정합니다.' },
            { term: '📈 일별 총 출고 건수 추이 (선 그래프)', desc: '가로 스크롤 없이 깔끔한 벡터 그래프로 날짜별 출고 건수 변화 흐름을 한눈에 파악합니다.' },
            { term: '🍩 주문 수량별 구성 비중 (원형 도넛 차트)', desc: '전체 주문 중 1개 주문, 2개 주문, 3개 이상 주문의 통합 비율(%)과 총 주문 건수를 도넛 차트로 제공합니다.' }
          ]
        },
        {
          heading: '📋 세부 데이터 테이블 & 통계 요약',
          items: [
            { term: '🔽 최신 날짜순 정렬', desc: '최신 출고표 데이터가 맨 위에 오도록 자동 정렬됩니다.' },
            { term: '📄 페이지네이션 (10개 / 15개씩 보기)', desc: '데이터가 많아진 경우 [10개씩], [15개씩] 보기 옵션과 하단 페이지 버튼으로 편리하게 조회합니다.' },
            { term: '📊 하단 고정 요약 행 (총 합계 & 일평균)', desc: '표 맨 아래에 선택한 조회 기간 전체의 [조회 기간 총 합계] 및 [📊 조회 기간 일평균 (건/일)] 요약 정보가 상시 표시됩니다.' }
          ]
        }
      ]
    },
      ...
      content: [
        {
          ...
          items: [
            { term: '플레인 (Base Yogurt)', desc: '모든 플레이버 제품의 기준이 되는 베이스 요거트입니다 (예: 플레인 100g, 목표수율 31.5%).' },
