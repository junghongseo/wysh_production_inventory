...
const UserManualView = () => {
  ...
  const manualData = useMemo(() => [
    ...
    // -------------------------------------------------------------
    {
      ...
      summary: '주간 생산 일정을 수립하고 요구르트/부재료 생산 계획을 생성합니다.',
      tags: ['신규 생산 계획', '계획 등록', '2종 동시', '플레인', '플레이버', '부재료', '차수 ID'],
      content: [
        {
        ...
        {
          heading: '📝 입력 항목 및 조건 가이드',
          items: [
            { term: '생산 구분 (요구르트 vs 부재료)', desc: '완제품 요구르트 생산과 자체 부재료(라즈베리 페이스트 등) 생산을 구분합니다.' },
            { term: '생산 방식 (단종 vs 2종 동시)', desc: '단일 발효조에서 1가지 제품을 생산할지, Base 발효 후 2가지 플레이버로 분할 병입할지 선택합니다.' },
            { term: '목표 주문량 / 마케팅 / 버퍼 수량', desc: '필요 수량을 입력하면 총 예정 생산량(Total Qty)이 자동 계산됩니다.' }
          ]
        },
        {
          heading: '🔍 드롭다운 옵션 및 필터 규칙',
          items: [
            { term: '생산 제품 드롭다운', desc: '등록된 전체 제품 중 생산 가능한 완제품만 노출됩니다.' },
            { term: '발효조 선택 (1~4호기)', desc: '현재 사용 가능한 발효 탱크 용량(L)을 선택합니다.' }
          ]
        },
        {
          heading: '⚡ 데이터 연동 구조',
          items: [
            { term: '차수 ID 자동 생성', desc: '생산 계획 등록 시 "YYYYMMDD-순번" 형태의 고유 차수 ID가 생성되어 재고 및 리포트 전체에 연동됩니다.' },
            { term: '출고기한 & 소비기한 산출', desc: '병입 예정일 기준 출고기한(+7일)과 소비기한(+22일)이 자동 계산되어 저장됩니다.' }
          ]
        }
      ]
    },
    {
      ...
      categoryId: 'calendar',
      mode: 'admin',
      title: '👑 [관리자 전용] 원재료 배합표 보기·인쇄 & 생산 계획 수정/삭제 관리',
      summary: '관리자 권한으로 원재료 배합 비율표를 출력하거나 등록된 생산 계획을 변경 및 삭제합니다.',
      tags: ['배합표 보기', '배합표 인쇄', 'PDF 출력', '계획 수정', '계획 삭제', '관리자 권한'],
      content: [
        {
          heading: '🔘 주요 관리자 전용 기능',
          items: [
            { term: '🖨️ [배합표 보기 / 인쇄 / PDF]', desc: '생산계획 상세 팝업 하단에 표시되며, 선택한 생산 계획의 원재료 및 부재료 배합 비율표를 슬라이드 드로어로 열어 출력하거나 PDF로 저장합니다 (레시피 보안을 위해 관리자 모드 전용 제공).' },
            { term: '✏️ [계획 수정]', desc: '생산계획 상세 팝업 하단의 수정 버튼을 눌러 목표 수량, 시작일, 발효조 정보를 변경합니다.' },
            { term: '🗑️ [계획 삭제]', desc: '더 이상 진행하지 않는 생산 계획을 완전 삭제합니다. 연동된 재고 기록도 함께 정돈됩니다.' }
          ]
        }
      ]
    },
    ...
    // -------------------------------------------------------------
    {
      ...
      tags: ['발효 리포트', '유청분리 리포트', '병입 리포트', '작성자 서명', 'pH', '수율'],
      content: [
        {
          heading: '🔍 단계별 드롭다운 생산 계획 노출 필터 규칙 (핵심)',
          items: [
            { term: '1단계: 발효 리포트', desc: '이번 주 활성화된 요구르트 생산 계획 전체가 드롭다운에 표시됩니다.' },
            { term: '2단계: 유청분리 리포트', desc: '발효 리포트는 작성 완료되었으나, 아직 유청분리 리포트가 작성되지 않은 생산 계획만 드롭다운에 노출됩니다.' },
            { term: '3단계: 병입 리포트', desc: '유청분리 리포트까지 작성 완료되었으나, 아직 병입 리포트가 작성되지 않은 생산 계획만 선택 가능하도록 노출됩니다.' }
          ]
        },
        {
          heading: '🧪 유청분리 리포트 주요 측정 항목',
          items: [
            { term: 'pH 수치 측정', desc: '제품별 적정 pH 범위를 실시간 게이지로 시각화하여 정상/경고 여부를 즉시 파악합니다.' },
            { term: '밧드 개수 & 마지막 밧드 무게', desc: '총 추출 유청 요구르트 무게(g) 및 로스율(%)을 자동 산출합니다.' }
          ]
        },
        {
          heading: '🍾 병입 리포트 수율 & 소비기한 산출 공식',
          items: [
            { term: '목표 수율 (Target Yield)', desc: '플레이버 제품의 경우 베이스 요구르트 제품의 목표 수율(예: 28%)이 표시됩니다.' },
            { term: '실제 수율 (Actual Yield)', desc: '플레이버 제품 생산 시 요구르트 함량 비율(Base Ratio, 예: 70%)을 곱해 베이스 환산 무게를 구한 뒤 원재료 대비 수율을 정확히 산출합니다.' },
            { term: '입고 수량 산출', desc: '실제 입고량 = 병입 수량 - (샘플/차감 수량)' },
            { term: '소비기한 2줄 표기', desc: '2종 동시 생산 시 각 품목별 소비기한(병입일+22일)이 각각 독립된 줄로 표시됩니다.' }
          ]
        }
      ]
    },
    {
      ...
      tags: ['확인 처리', '미확인', '확인완료', '재고 자동 반영', '음영 처리'],
      content: [
        {
          heading: '🔘 승인 프로세스 & 시각적 구별',
          items: [
            { term: '⚠️ 미확인 (작업자 제출 직후)', desc: '작업자가 작성한 리포트는 연한 오렌지 음영 배경과 [⚠️ 미확인] 뱃지가 표시됩니다.' },
            { term: '✓ [확인 처리] 버튼 (관리자 전용)', desc: '관리자 로그인 상태에서 이력 카드의 [✓ 확인 처리] 버튼을 누르면 단 1회의 승인 동의 후 확인 완료 처리됩니다.' },
            { term: '✓ 확인완료', desc: '승인 후 정상 배경과 [✓ 확인완료] 뱃지로 변경되며, 서버 동기화 후에도 확인 상태가 영구 유지됩니다.' }
          ]
        },
        {
      ...
      content: [
        {
          ...
          items: [
            { term: '계획 수량 vs 실제 입고', desc: '초기 계획된 수량과 승인된 병입 리포트에서 자동 연동된 실제 입고 수량을 비교합니다.' },
            { term: '현재 재고', desc: '현재 재고 = 실제 입고량 - 총 출고(사용) 수량. 100개 미만 시 붉은색 강조 표출.' },
            { term: '연동 상태 뱃지', desc: '병입 리포트 승인 여부에 따라 [✓ 병입 입고 연동완료] 또는 [리포트 승인 대기]로 표출됩니다.' }
          ]
        },
        {
          heading: '📝 사용(출고) 입력 규칙',
          items: [
            { term: '출고 차수 선택 드롭다운', desc: '출고기한 내에 있고, 현재 재고가 1개 이상 남아있는 생산 차수/품목만 드롭다운에 노출됩니다.' },
            { term: '출고 수량 제한', desc: '현재 남은 재고 수량을 초과하는 수량은 출고할 수 없도록 자동 검증됩니다.' },
            { term: '미래 일자 경고', desc: '출고 일자를 오늘 이후로 선택 시 경고 알림창을 통해 재확인합니다.' }
          ]
        }
      ]
    },
      ...
      content: [
        {
          ...
          items: [
            { term: '플레인 (Base Yogurt)', desc: '모든 플레이버 제품의 기준이 되는 베이스 요구르트입니다 (예: 플레인 100g, 목표수율 28%).' },
            { term: '플레이버 (Flavor Yogurt)', desc: '베이스 요구르트에 과일/부재료가 혼합된 제품입니다. 베이스 제품 지정 및 요구르트 비율(예: 70%)을 설정합니다.' },
      ...
      content: [
        {
          ...
          items: [
            ...
            { term: '💬 대화 모드 / 질의응답', desc: '"현재 플레인 요구르트 재고 얼마 남아있어?", "유청분리 로스율 계산 방법 알려줘" 등 자연어로 질문할 수 있습니다.' }
          ]
        }
      ]
    }
      ...
      <div className="glass-card" style={{ padding: '24px', borderRadius: '16px', background: 'linear-gradient(135deg, rgba(2, 132, 199, 0.08), rgba(99, 102, 241, 0.05))', border: '1px solid rgba(2, 132, 199, 0.2)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
          <div>
            ...
            <p style={{ margin: '6px 0 0 0', fontSize: '0.88rem', color: 'var(--text-secondary)' }}>
              모드별(일반 작업자 / 관리자) 상세 사용 설명, 버튼 기능, 드롭다운 필터 규칙 및 수율·소비기한 연동 공식을 검색할 수 있습니다.
            </p>
          </div>
        ...
        <div style={{ marginTop: '20px', position: 'relative' }}>
          ...
          <input
            ...
            placeholder="버튼명, 작성항목, 수율, 소비기한, 출고, 리포트, 배합표 등 궁금한 키워드를 입력하세요..."
            value={searchQuery}
